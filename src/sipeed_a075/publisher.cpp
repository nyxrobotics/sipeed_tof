...
void timer_callback()
{
  ...
  // printf("%s:%d\n",__FILE__,__LINE__);
  auto tempcali =
      CaliInst->TOF_cali(data_out3, std::vector<uint16_t>((uint16_t*)oldframe.status,
                                                          (uint16_t*)((uint8_t*)oldframe.status) + sizeof(Image_t)));
  // printf("%s:%d\n",__FILE__,__LINE__);
  auto colormap_temp =
      CaliInst->MapRGB2TOF(tempcali, std::vector<uint8_t>((uint8_t*)oldframe.rgb,
                                                          (uint8_t*)((uint8_t*)oldframe.rgb) + sizeof(oldframe.rgb)));
  // printf("%s:%d\n",__FILE__,__LINE__);
  auto tempcali_ir = CaliInst->TOF_cali(
      std::vector<uint16_t>((uint16_t*)oldframe.ir, (uint16_t*)((uint8_t*)oldframe.ir) + sizeof(Image_t)),
      std::vector<uint16_t>((uint16_t*)oldframe.status, (uint16_t*)((uint8_t*)oldframe.status) + sizeof(Image_t)));
  ...
  sensor_msgs::Image rgbmsg = *cv_bridge::CvImage(std_msgs::Header(), "bgr8", mRGB_bgr).toImageMsg().get();
