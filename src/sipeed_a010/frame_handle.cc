...
__find_header:
  ...
  if (vecChar.size() < FRAME_HEAD_SIZE + frame_payload_len + FRAME_CHECKSUM_SIZE + FRAME_END_SIZE)
  {
  ...
  {
    uint8_t check_sum =
        std::accumulate(vecChar.begin(), vecChar.begin() + FRAME_HEAD_SIZE + frame_payload_len, (uint8_t)0);
  ...
  std::vector<uint8_t>(vecChar.begin() + FRAME_HEAD_SIZE + frame_payload_len + FRAME_CHECKSUM_SIZE + FRAME_END_SIZE,
                       vecChar.end())
      .swap(vecChar);
  return pf;
